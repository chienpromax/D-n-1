...
import watersys.BanHang.ChiTietSPFrom1;
...
public class BanForm extends javax.swing.JInternalFrame {
    ...
    void openThemSP() {
        try {
            ...
            ChiTietSPFrom1 chiTietSPForm = new ChiTietSPFrom1(parentFrame, true);
...
// Phương thức được gọi khi form ChiTietSPFrom đóng
    ...
    HoaDon getTenSPGia() {
        ...
        //khởi tạo một đối tượng HashMap trong Java với khóa là String và giá trị là Integer. 
        ...
        // trả về tất cẩ các khóa trong map 
        //.stream().collect  thực hiện các thao tác có thể thay đổi trên các phần tử được lưu giữ trong Stream.
        model.setTenpSP(mapTenSPSoLuong.keySet().stream().collect(Collectors.joining(", ")));
        //sử dụng để chuyển đổi các phần tử của luồng thành kiểu dữ liệu số nguyên (int)
...
//    ArrayList<String> maBan = new ArrayList<>();
    ...
    //Là một lớp được triển khai từ giao diện Printable, có nhiệm vụ định nghĩa cách vẽ hóa đơn khi in.
    public class BillPrintable implements Printable {
        private List<Object[]> billData;
        public BillPrintable(List<Object[]> billData) {
            this.billData = billData;
        }
        //Được gọi khi in mỗi trang.
...
//            ImageIcon icon = new ImageIcon("E:\\hoc\\kì 4\\Du_an_1_(UDPM-Java)\\WaterSys\\src\\watersys\\Icon\\yea.jpg");
            ...
            if (pageIndex == 0) {
                ...
                //Sử dụng Graphics2D để vẽ nội dung hóa đơn.
                ...
                try {
                    int y = 20;//trên xuống
                    int yShift = 10;
                    //Mỗi yShift thể hiện khoảng cách giữa các dòng.
                    int headerRectHeight = 15;
                    g2d.setFont(new Font("Monospaced", Font.PLAIN, 12));
//                    g2d.drawImage(icon.getImage(), 70, 60, 20, 20, rootPane);
                    y += yShift + 30;
                    g2d.drawString("--------------------------------------", 10, y);
                    // drawString vẽ 1 cuỗi văn bản lên đối tượng đồ họa
                    y += yShift;
...
//            TongTien = Float.parseFloat(firstRow[5].toString());
            ...
            // Xử lý in hóa đơn
            //PrinterJob là một lớp trong Java cung cấp các phương thức để quản lý các công việc in.
    ...
    public PageFormat getPageFormat(PrinterJob pj) {
        ...
        //setImageableArea được sử dụng để đặt vùng in của trang giấy.
        pf.setOrientation(PageFormat.PORTRAIT);//để đặt hướng in là dọc 
        pf.setPaper(paper);
        return pf;
    }
    //đổi cm sang pixels
